...
discCanvas.width  = boardCanvas.width  = CW;
discCanvas.height = boardCanvas.height = CH;
...
function drawDiscCanvas() {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      const val = board[r][c];
      if (!val) continue;
      if (fallingDisc && fallingDisc.targetRow === r && fallingDisc.col === c) continue;
      const { x, y } = cellCenter(r, c);
...
function startFallAnimation(col, player, targetRow) {
  ...
  fallingDisc = { col, player, targetRow, targetY, currentY: -RADIUS };
  speelGeluid('Score_04.mp3');
  animatefall();
}
...
function animatefall() {
  ...
  if (dist > 1) {
    ...
    if (fallingDisc.currentY >= targetY) {
      ...
      drawAll();
      fallingDisc = null;
      if (winningCells.length > 0) startPulseLoop();
      return;
    }
  } else {
    fallingDisc.currentY = targetY;
...
function updateColButtons() {
  colButtons.forEach((btn) => {
    ...
    if (myTurn) btn.classList.add(myPlayer === 1 ? 'active-player-1' : 'active-player-2');
  });
}
...
function getTurnMessage() {
  return currentPlayer === myPlayer ? '⚡ Jouw beurt!' : '⏳ Wachten op tegenstander...';
}
