...
memoryNS.on('connection', (socket) => {
  ...
  socket.on('kiesGrid', (gridSize) => {
    ...
    memFase           = 'playing';
    const totaalParen = (gridSize * gridSize) / 2;
    memStatus = {
      kaarten:       maakSpeelveld(gridSize),
      pogingen:      0,
      gevondenParen: 0,
      totaalParen,
      spelKlaar:     false,
  ...
  socket.on('draaiKaart', (kaartId) => {
    if (memFase !== 'playing') return;
    if (socket.id !== memBeurt || memStatus.spelKlaar) return;
    ...
    if (omgedraaid.length === 2) {
      ...
      if (k1.id === k2.id) {
        ...
        setTimeout(() => {
          k1.omgedraaid = k2.omgedraaid = false;
          const ids = Object.keys(memSpelers);
          memBeurt  = ids.find(id => id !== socket.id) || socket.id;
          memStuurSpelUpdate();
        }, 1200);
      }
    }
...
stokvangenNS.on('connection', (socket) => {
  ...
  socket.on('throwStick', (stickIndex) => {
    ...
    setTimeout(() => {
      if (stokState.phase === 'catching' && stokState.fallingStick?.index === stickIndex && stokState.fallingStick?.catcherRole === catcherRole) {
  ...
  socket.on('naarHome', () => {
    stokvangenNS.emit('stuurNaarHome');
  });
...
fourNS.on('connection', (socket) => {
  ...
  socket.emit('assigned', {
    ...
    currentPlayer: fourState.currentPlayer,
    gameStarted:   fourState.connectedPlayers === 2,
  });
  ...
  socket.on('move', ({ col }) => {
    ...
    if (!fourState.gameStarted || fourState.winner) return;
    if (fourState.currentPlayer !== player) return;
    ...
    if (winningCells) {
      fourState.winner       = player;
      fourState.winningCells = winningCells;
      fourNS.emit('gameOver', { board: fourState.board, winner: player, winningCells, lastMove: { row: targetRow, col, player } });
      return;
    }

    if (fourCheckDraw(fourState.board)) {
      fourNS.emit('draw', { board: fourState.board });
      return;
    }
  ...
  socket.on('restart', () => {
    fourResetGame();
    fourNS.emit('restart', { board: fourState.board, currentPlayer: fourState.currentPlayer });
  });

  socket.on('naarHome', () => {
    fourNS.emit('stuurNaarHome');
  });
...
puzzelNS.on('connection', (socket) => {
  if (puzzelState.playerIds.length === 0) {
    puzzelState = freshPuzzelState();
  }

  if (puzzelState.playerIds.length >= 2) {
    socket.emit('vol', 'Het spel is al vol.');
    socket.disconnect();
    return;
  }
  ...
  socket.on('kiesGrid', (size) => {
    if (puzzelState.spelers[socket.id]?.rol !== 'player1') return;
    if (![2, 3, 4, 5].includes(size)) return;
    puzzelState.gridSize = size;
    puzzelNS.emit('gridGekozen', size);
  });
  ...
  socket.on('kiesFoto', (foto) => {
    ...
    if (!speler) return;
    speler.foto = foto;
    socket.emit('fotoBevestigd', foto);
    const iedereen = puzzelState.playerIds.every(id => puzzelState.spelers[id]?.foto);
    puzzelNS.emit('fotoStatus', iedereen);
  });

  socket.on('startSpel', () => {
    if (puzzelState.spelers[socket.id]?.rol !== 'player1') return;
    if (puzzelState.playerIds.length < 2) return;
    const iedereen = puzzelState.playerIds.every(id => puzzelState.spelers[id]?.foto);
    if (!iedereen) return;
    puzzelState.phase     = 'playing';
    puzzelState.startTijd = Date.now();
    const seed = puzzelState.startTijd;
    puzzelState.playerIds.forEach(id => {
      const sp = puzzelState.spelers[id];
      puzzelNS.to(id).emit('spelStart', { foto: sp.foto, gridSize: puzzelState.gridSize, seed });
    });
  });
  ...
  socket.on('stukGeplaatst', (count) => {
    ...
    for (const sp of Object.values(puzzelState.spelers)) {
      counts[sp.rol] = sp.geplaatst;
    }
    ...
    if (count >= totaal && !speler.klaar) {
      speler.klaar = true;
      speler.tijd  = Date.now() - puzzelState.startTijd;

      if (!puzzelState.eersteKlaar) {
        puzzelState.eersteKlaar = speler.rol;
        puzzelNS.emit('eersteKlaar', { winnaar: speler.rol, tijd: speler.tijd });
      } else {
        puzzelNS.emit('tweedeKlaar', { speler: speler.rol, tijd: speler.tijd });
      }
    }
  });

  socket.on('restartGame', () => {
    puzzelState.phase       = 'selecting';
    puzzelState.foto        = null;
    puzzelState.gridSize    = 2;
    puzzelState.startTijd   = null;
    puzzelState.eersteKlaar = null;

    for (const id of puzzelState.playerIds) {
      puzzelState.spelers[id].geplaatst = 0;
      puzzelState.spelers[id].klaar     = false;
      puzzelState.spelers[id].tijd      = null;
      puzzelState.spelers[id].foto      = null;
    }

    puzzelNS.emit('naarSelectie');
    puzzelNS.emit('spelersCount', puzzelState.playerIds.length);
    puzzelNS.emit('gridGekozen', puzzelState.gridSize);
  });

  socket.on('naarHome', () => {
    puzzelNS.emit('stuurNaarHome');
  });
...
// Stuur de reeks aan iedereen en start dan input fase
function kfToonReeksEnStart() {
  kleurenflitsState.phase = 'showing';

  // Reset input state voor alle spelers
  ...
  kleurenflitsNS.emit('toonReeks', {
    reeks: kleurenflitsState.reeks,
    ronde: kleurenflitsState.ronde,
  });

  // Na het tonen mogen alle spelers tegelijk invoeren
...
// Nieuwe ronde: voeg kleur toe en toon reeks
function kfNieuweRonde() {
  kleurenflitsState.ronde++;
  const nieuweKleur = KF_KLEUREN[Math.floor(Math.random() * 4)];
  kleurenflitsState.reeks.push(nieuweKleur);
  kfToonReeksEnStart();
}

// Check of beide spelers klaar zijn → volgende ronde
function kfCheckRondeKlaar() {
  const allemaal = kleurenflitsState.playerIds.every(id =>
    kleurenflitsState.spelers[id].rondeDone
  );
  if (!allemaal) return;
...
// Stuur de reeks opnieuw aan één speler zodat hij het opnieuw kan proberen
function kfHerproberenVoorSpeler(socketId) {
  ...
  kleurenflitsNS.to(socketId).emit('herproberenDirect', {
    ronde: kleurenflitsState.ronde,
    reeks: kleurenflitsState.reeks,
  });
}
...
kleurenflitsNS.on('connection', (socket) => {
  if (kleurenflitsState.playerIds.length === 0) {
    kleurenflitsState = freshKleurenflitsState();
  }
  ...
  kleurenflitsState.spelers[socket.id] = {
    rol,
    levens:     KF_LEVENS,
    inputIndex: 0,
    rondeDone:  false,
  ...
  socket.on('drukKnop', (kleur) => {
    ...
    if (kleur === verwacht) {
      ...
      if (speler.inputIndex >= kleurenflitsState.reeks.length) {
        // Hele reeks goed → deze speler is klaar
        speler.rondeDone = true;
        socket.emit('reeksVoltooid');

        // Vertel de andere speler dat deze klaar is
        const andereId = kleurenflitsState.playerIds.find(id => id !== socket.id);
        if (andereId) {
          kleurenflitsNS.to(andereId).emit('tegenstallerKlaar', { rol: speler.rol });
        }
      ...
      // Vertel de andere speler dat deze een fout maakte
      const andereId = kleurenflitsState.playerIds.find(id => id !== socket.id);
      if (andereId) {
        kleurenflitsNS.to(andereId).emit('tegenstallerFout', { rol: speler.rol, levens: speler.levens });
      }
      ...
      if (speler.levens <= 0) {
        ...
        const winnaarRol = winnaarId ? kleurenflitsState.spelers[winnaarId].rol : null;
        kleurenflitsNS.emit('gameOver', { winnaar: winnaarRol, ronde: kleurenflitsState.ronde });
      } else {
        // Nog levens → toon reeks opnieuw alleen aan deze speler
        setTimeout(() => kfHerproberenVoorSpeler(socket.id), 1500);
      }
    }
  });
  ...
  socket.on('restartGame', () => {
    ...
    kleurenflitsState.reeks  = [];
    kleurenflitsState.ronde  = 0;
    kleurenflitsState.phase  = 'playing';
  ...
  socket.on('naarHome', () => {
    kleurenflitsNS.emit('stuurNaarHome');
  });
